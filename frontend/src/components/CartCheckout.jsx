      ...
      {v.cartHasItems && (
        <div style={sx`display:grid;grid-template-columns:var(--shop);gap:24px;align-items:start;direction:rtl`}>
          ...
          <aside style={sx`grid-column:2;order:1;position:sticky;top:150px;background:#fff;border:1px solid #E9E3DA;border-radius:14px;padding:20px;display:flex;flex-direction:column;gap:12px`}>
            ...
            <div style={sx`display:flex;justify-content:space-between;font-size:14px;color:#4A453E`}><span>الشحن</span><strong>{v.shippingText}</strong></div>
            <div style={sx`display:flex;gap:8px;margin-top:4px`}>
              <input type="text" value={v.coupon} onChange={v.onCoupon} placeholder="كود الخصم (TEST10)" aria-label="كود الخصم" style={sx`flex:1;height:44px;border:1px solid #E1DACE;border-radius:10px;background:#FBF9F6;padding:0 12px;font-size:13px`} />
            ...
            {v.couponMsg && <span style={sx`font-size:12.5px;color:${v.couponMsgColor}`}>{v.couponMsg}</span>}
...
export function CheckoutPage({ v }) {
  return (
    <section style={sx`max-width:1100px;margin:0 auto;padding:26px var(--pad) 60px`}>
      {v.orderDone && (
        <div style={sx`background:#fff;border:1px solid #E9E3DA;border-radius:16px;padding:56px 26px;text-align:center;display:flex;flex-direction:column;align-items:center;gap:12px;animation:popIn .3s ease both`}>
          ...
          <p style={sx`margin:0;font-size:14.5px;color:#7C766D`}>رقم الطلب <strong style={sx`color:#1E1B18`}>{v.orderId}</strong> — سنتواصل معك لتأكيد التوصيل خلال {v.orderEta}.</p>
          <div style={sx`display:flex;gap:10px;flex-wrap:wrap;justify-content:center;margin-top:10px`}>
        ...
        <>
          ...
          <div style={sx`display:grid;grid-template-columns:var(--pdp);gap:24px;align-items:start`}>
            <form onSubmit={v.placeOrder} style={sx`display:flex;flex-direction:column;gap:18px`}>
              <div style={sx`background:#fff;border:1px solid #E9E3DA;border-radius:14px;padding:20px;display:flex;flex-direction:column;gap:14px`}>
                ...
                <label style={sx`display:flex;flex-direction:column;gap:6px;font-size:13px;font-weight:600`}>منطقة التوصيل
                  <select value={v.ck.city} onChange={v.setCity} style={sx`height:48px;border:1px solid #E1DACE;border-radius:10px;background:#FBF9F6;padding:0 13px;font-size:14px;font-weight:400;cursor:pointer`}>
              ...
              <div style={sx`background:#fff;border:1px solid #E9E3DA;border-radius:14px;padding:20px;display:flex;flex-direction:column;gap:12px`}>
                ...
                <label style={sx`display:flex;align-items:flex-start;gap:10px;font-size:13px;color:#4A453E;cursor:pointer;margin-top:4px`}><input type="checkbox" checked={v.ck.terms} onChange={v.setTerms} style={sx`width:17px;height:17px;accent-color:#1F4E4A;margin-top:2px;cursor:pointer`} /><span>أوافق على <a href="#/terms">الشروط والأحكام</a> و<a href="#/return-policy">سياسة الإرجاع</a></span></label>
            ...
            <aside style={sx`position:sticky;top:150px;background:#fff;border:1px solid #E9E3DA;border-radius:14px;padding:20px;display:flex;flex-direction:column;gap:11px`}>
              ...
              <div style={sx`display:flex;justify-content:space-between;font-size:13.5px;color:#4A453E`}><span>المجموع الفرعي</span><strong>{v.subtotalText}</strong></div>
              {v.hasDiscount && <div style={sx`display:flex;justify-content:space-between;font-size:13.5px;color:#2E7D5B`}><span>الخصم</span><strong>−{v.discountText}</strong></div>}
              <div style={sx`display:flex;justify-content:space-between;font-size:13.5px;color:#4A453E`}><span>الشحن ({v.areaName})</span><strong>{v.shippingText}</strong></div>
              <div style={sx`border-top:1px solid #F0EBE3;padding-top:11px;display:flex;justify-content:space-between;align-items:baseline`}><span style={sx`font-size:15px;font-weight:800`}>الإجمالي</span><strong style={sx`font-size:22px;color:#1F4E4A`}>{v.totalText}</strong></div>
            </aside>
          </div>
        </>
      )}
