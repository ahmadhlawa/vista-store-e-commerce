...
export function OrderDetailPage() {
  ...
  const reopen = async () => { if (!reopenReason.trim()) { feedback.error("سبب إعادة الفتح مطلوب."); return; } setBusy(true); try { const row = await adminApi.reopenOrder(orderId, reopenReason.trim()); setOrder(row); setDraft(initialDraft(row)); setNextStatus(row.status); setReopenReason(""); feedback.success("أعيد فتح الطلب واستُبدلت الفاتورة النشطة."); } catch (error) { feedback.error(error.message || "تعذّر إعادة فتح الطلب."); } finally { setBusy(false); } };
