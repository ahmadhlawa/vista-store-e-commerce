...
import {
  ...
  isMoney, isWholeQuantity, orderSourceLabels, orderStatusLabels, paymentMethodLabels, paymentStatusLabels,
...
function CatalogItemsEditor({ items, products, onChange, disabled }) {
  ...
  return <section aria-label="أصناف الطلب" style={sx`display:flex;flex-direction:column;gap:10px`}>
    {items.map((item, index) => <fieldset key={`${item.product_id}-${item.variant_id || "base"}`} disabled={disabled} style={sx`border:1px solid #E4E0D9;border-radius:10px;padding:12px;display:grid;grid-template-columns:minmax(130px,1fr) 90px 120px auto;gap:10px;align-items:end`}>
...
const initialDraft = (row) => ({
  ...
  items: (row.items || []).filter((item) => item.product_id).map((item) => ({ ...item, kind: "catalog", quantity: asText(item.quantity), unit_price: asText(item.unit_price) })),
});

export function OrderDetailPage() {
  const { orderId } = useParams(); const navigate = useNavigate(); const { admin } = useAdminAuth(); const feedback = useFeedback();
  const [order, setOrder] = useState(null); const [draft, setDraft] = useState(null); const [products, setProducts] = useState([]); const [loading, setLoading] = useState(true); const [busy, setBusy] = useState(false); const [nextStatus, setNextStatus] = useState(""); const [statusNote, setStatusNote] = useState(""); const [confirming, setConfirming] = useState(false); const [completing, setCompleting] = useState(false);
